...
// ADDING A NEW ITEM TO THE LIST
  ...
  var addItem = function() {
    ...
    var nameDespacho = $("#nameDespacho").val();
    
    // If the input text field isn't empty, add it to the list as a new item
    if (input) {
      $('.list').prepend('<li class="list__item">' + input + '</li>');
      fetch("https://desarrolloenviexpress.site/e-control/ajax/download_in_cedi/download_in_cedi.php?code=" + input + '&nameUsuario=' + nameUsuario + '&nameDespacho=' + nameDespacho).then(response => {
