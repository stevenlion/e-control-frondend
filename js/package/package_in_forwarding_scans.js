...
// ADDING A NEW ITEM TO THE LIST
  ...
  var addItem = function() {
    ...
    var namePlaca = $("#namePlaca").val();
    var nameUsuario = $("#nameUsuario").val();

    // If the input text field isn't empty, add it to the list as a new item
    if (input) {
      $('.list').prepend('<li class="list__item">' + input + '</li>');
      fetch("https://desarrolloenviexpress.site/e-control/ajax/package/package_in_forwarding_scan.php?NameCodigoBarras=" + input + '&nameConsecutivo=' + nameConsecutivo + '&namePlaca=' + namePlaca + '&nameUsuario=' + nameUsuario ).then(response => {
